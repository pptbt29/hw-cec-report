...
第四类是长期成本感知路由与 KV Manager 联合方案。该策略使用与纯长期策略相同的路由逻辑，并进一步启用 block-level 缺失块同步和请求完成后触发的后台主动 KV placement。默认配置下，纯长期策略关闭 block-level 机制且只执行整段被动 KV 恢复，因此二者差异表示 KV Manager 的总体增益；通过为纯长期策略单独打开 block-level 开关，还可以进一步分离块级同步和后台主动放置的贡献。
...
第六，运行长期成本感知路由与 KV Manager 联合方案，在长期策略基础上加入 block-level 缺失块同步和后台主动 KV placement，并比较其相对纯长期路由的前台同步量、后台放置流量、KV 命中率和服务中断风险；随后打开纯长期策略的 block-level 开关，进一步分离两项机制的独立贡献。
