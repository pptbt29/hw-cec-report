...
- 携带 **SLA、配置组、到达时间、prefix hash** 等调度所需字段。
...
mobility_transitioned 当前请求是否真的发生入口迁移
...
WorkloadGroup:
  ...
  name              配置组名（可用 high/normal，也可使用任意业务名）
  entry_mode        counts | ratios
  concurrency       ratios 模式下的组总并发
  entry_concurrency counts 模式下各入口具体人数
...
3. 应用移动性：按 request/session/markov 语义更新移动窗口后的 entry_node。
...
- `request`：后半段每个请求相对原始 home 独立抖动。
- `session`：被选中的 session 一次迁到固定新入口，后续不再变化。
- `markov`：维护当前入口，在该入口累计驻留 `mobility_residency_turns` 轮后，才按
  `mobility_ratio` 尝试迁往其他入口。
...
- `summary()` 输出：请求总数、各模型/配置组数量、平均/分位 prompt 与 output 长度、移动请求比例、平均会话轮数、预计总 KV token 量，供 sanity check。
