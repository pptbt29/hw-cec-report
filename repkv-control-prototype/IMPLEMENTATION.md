...
DRAM 与对端副本没有预留。判定「当前布局已经能使 TTFT 满足 SLO」时，把 restore/transfer 路径按副本仍在的概率与「只靠本地 HBM、其余 recompute」混合。占用低于低水位且同一 session 在两台以上机器上有完整副本时，该概率为 1。唯一副本只在当场恢复已经占掉预算一半以上时才打折；restore 相对 SLO 很宽裕时仍选空动作，避免去占 host 链路。占用高或恢复已经贴着预算时，最小前缀会把前缀钉进本地 HBM，集群概率也会给第二份 durable 副本正增益。
