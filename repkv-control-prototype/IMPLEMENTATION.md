...
方法代价定义为每 block 消耗的资源秒 $1/R_m$，因此规划器最小化的是前台要争的那个量，而不是人为的偏好顺序。
...
`run.py` 的默认实验配置对应 `PROTOTYPE_VERDICT.md` 第 6.1 节的数据中心主配置：4 节点、存活并发 90、上下文 32000 token、SLO 3.0 秒、DRAM 层 256 GiB。该配置位于"取回可行、重算不可行"区间，三种策略在此分开。`--ttft-slo` 可以跨过两个门槛，`--network-gbytes` 与 `--host-gib` 可以移动门槛本身。`--lru-eviction` 把回收退回最近使用时间，`--min-return-probability` 提到 1 以上则关闭全部准备，两者合起来构成准备/回收的 $2\times2$ 消融。不做 session 补充时不产生排队竞争（见第 14 节第 1 条）。
...
剩余恢复时间对目标前缀单调不增，因此可行性对目标前缀单调，最小可行 $X_{s,n}$ 用二分求得。如果节点当前已经满足 SLO，则不为该节点准备；如果 $Q_n+T_s^{prompt}>D_s$，即使完整 KV 在本地也无法满足 SLO，也不生成计划。最小可行目标若仍无法在 $\widehat t_s-t$ 内建完，则更大的目标同样来不及，计划器直接放弃。

`eager_full` 不经过这两条否决。只要思考时间内还能建完，它就把整段准备到另一台，净收益近似为 $1-0.01C$，几乎总为正。最小前缀则在「预测已经能赶上」或「预测排队已经单独超限」时直接放弃；即便生成了计划，还要再过 $V_{s,n}>0$ 的过滤。原机 DRAM restore 已经被判为可行时，再向第二台机器准备的集群概率增量接近 0，过滤会再挡掉一轮。因此同一套 LRU 回收下，完整准备愿意动手的节点次数可以比最小前缀多出一个数量级。差距来自准备准入，不是回收规则。
...
还统计后续轮 attainment、P99 TTFT、每个成功请求对应的 transfer/restore/recompute blocks、每个成功请求对应的 HBM block-seconds、未使用准备比例、降级 blocks、取消 batches、重叠轮次、推迟接纳次数和脚本池耗尽次数。另外单独报告准备准入：对每个空闲 `(session, node)` 是否生成计划、是否因「预测已经能赶上」或「预测排队超限」放弃、是否被净收益过滤掉，以及真正动手时目标前缀占整段的比例、被下一问用掉的准备量和到达时走前台网络搬运的量。完整准备与最小前缀的差距首先出现在这些准入计数上，而不是回收计数上。
...
12. 第 8.1 节的排队预测在高利用率下系统性偏低，导致 $P_s^{cluster}$ 高估，随排队放大不确定性改善了分辨力但没有修正偏差；物理模型下排队的主要来源已从设备串行队列变为 decode 槽位等待，但仍是同一形式的流体近似；
13. 节点是同质的。算力异构会使重算门槛成为节点属性，当前没有实现；
...
2. 实现节点异构，使重算门槛成为节点属性，并把放置与路由合为一个决策；
3. 在 workload 中加入会话迁移，检验亲和被迫打破时预放置是否成为唯一手段；
4. 替换第 8.1 节的排队预测。当前形式按加权平均负载求确定性队列，而排队延迟对负载是凸函数，因此结果小于队列的期望值。需要给出队列长度的分布而不是均值，或对历史预测误差做在线校正。这一项同时影响准备判据和回收分数中的概率项；
5. 使计划的决策单位与记账单位一致：在接纳计划时预留其全部所需资源，使声称的收益可实现；
6. 重新审视最小前缀准备的定位。物理参数下它在取回可行区间弱于完整准备；在必须本地区间一旦动手则目标长度接近整段，但「预测已经能赶上」的否决使它经常不准备。需要给出它确有优势的条件，或将其降为次要贡献；
