...
流体队列会与到达时的实际排队、实际 TTFT 成对，用 EWMA 记下残差均值和散度。控制面随后使用的排队是流体值加上已观测残差；冷启动样本不足时不加这项，避免把先验标准差写进点估计。最小前缀的 `queue_exceeds_budget` 只用残差均值，不加散度倍数，以免把方差写成「排队已经单独超过 SLO」。集群可路由概率用均值加一倍散度。
