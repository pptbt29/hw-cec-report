...
这套估计有三处系统性偏乐观，最小前缀的硬否决会把它们写成空动作。第一，占用按 $q_s(t,\Delta)$ 折算后当作确定量推进。排队时延对负载凸，按平均负载得到的队列小于队列的期望（见 `PROTOTYPE_VERDICT.md` 第 12 节）。第二，恢复时间用空载服务时间 $B/R_m$，不投影网卡和 host 链路上已经排着的前台或后台；真实 TTFT 在 `_service_ttft` 里要在三条资源时间线和 decode 槽位上排队。第三，可行性用当前 HBM/DRAM 快照。DRAM 按最近使用时间淘汰，HBM 会降级，计划不预留这些副本，当时算得上的 restore 或 transfer 路径到达时可能已经没了。影子路由按当时的预测 TTFT 归属负载，真实 router 按到达时的准确状态选择节点，两者可以不一致。

最小前缀用 $\max(Q_n,T^{recover})+T^{prompt}$ 与 $D_s$ 做硬比较。用均值去回答「会不会超过 SLO」在类别上就不对：即便 $Q_n$ 无偏，方差一大仍会成批错过。集群可路由概率再用 sigmoid 把已经超过 SLO 的预测 TTFT 写成正成功概率，价值过滤会把第二份准备的增益看成接近 0。完整预恢复不经过这两条否决。
...
`eager_full` 不经过这两条否决。只要 think time 内还能建完，它就把整段准备到另一台，净收益近似为 $1-0.01C$，几乎总为正。最小前缀则在「预测 TTFT 已满足 SLO」或「预测排队已经单独超过 SLO」时直接放弃；即便生成了计划，还要再过 $V_{s,n}>0$ 的过滤。原机 DRAM restore 已经被判为可行时，再向第二台机器准备的集群概率增量接近 0，过滤会再挡掉一轮。因此同一套 LRU 回收下，完整准备愿意动手的节点次数可以比最小前缀多出一个数量级。差距来自准备准入，不是回收规则。
...
12. 第 8.1 节用来判定 TTFT 的预测量系统性偏乐观：流体排队低估、恢复按空载服务时间计、KV 快照到达前会变。最小前缀把该点估计写成硬否决，$P_s^{cluster}$ 的 sigmoid 再高估成功概率；随排队放大不确定性改善了分辨力但没有修正偏差；
...
4. 替换第 8.1 节的排队预测，并停止用点估计做硬否决。当前形式按加权平均负载求确定性队列，而排队延迟对负载是凸函数，因此结果小于队列的期望值。需要给出队列长度的分布而不是均值，或对历史预测误差做在线校正；准备准入应看超过 SLO 的概率，而不是把 $\widehat{TTFT}\le D_s$ 当成事实。这一项同时影响准备判据和回收分数中的概率项；
