...
方法代价定义为每 block 消耗的资源秒 $1/R_m$，因此规划器最小化的是前台要争的那个量，而不是人为的偏好顺序。重算与跨机搬迁是两个对等动作。各自在恢复预算 $\beta$ 内能赶完的最长上下文为 $C_m=\beta R_m t_{blk}$，再按长短排序：

$$C_1=\min(C_{\text{recompute}},C_{\text{transfer}}),\qquad
C_2=\max(C_{\text{recompute}},C_{\text{transfer}}).$$

$C_1\le C_2$ 恒成立。网快时 $C_2$ 是搬迁；网慢到搬迁比重算还慢时两者对调，中间区间仍然存在，只是剩下的动作变成重算。本机 restore 是第三个动作，有自己的 $C_{\text{restore}}$。`ResourceModel.thresholds` 同时返回各动作长度和排好序的 $C_1$、$C_2$。
