...
实验设计需要先围绕 SOW 中三项算法交付展开，而不是把泛化端-边-云场景全部铺开。建议将实验划分为三组主实验和一组协同实验。第一组是资源自适应模型切分实验，用于验证 layer-wise stage boundary、stage placement、pipeline balance 以及必要的 TP/EP/DP 补充能力是否优于多节点平均层级切分和固定并行模板；第二组是业务感知 batching 实验，用于验证在线参数自适应、请求重排和混合 batching 策略相对静态 batching/FIFO 的收益；第三组是用户移动场景下的请求调度实验，用于验证多流量入口、多推理实例、链路拓扑和 prefix/KV 状态共同约束下的路由策略是否优于就近实例路由；第四组才是协同实验，用于观察切分部署、batching 和移动请求调度共享状态后的叠加收益与稳定性。相关场景设置可以参考端-边-云协同智能、移动服务放置和 LLM serving benchmark 中对异构资源、移动性和 SLO/goodput 的评估方式 \cite{zhong2024distserve}, \cite{duan2022distributed}, \cite{ouyang2018follow}。

验证环境应尽量贴近任务书约束：OpenEulerOS、CANN 和任一可在 Ascend 上运行的推理框架，模型覆盖 CodeLlama34B 与 Qwen2-VL-7B-Instruct，硬件包含 ARM CPU、230T/64GB 或 70T/24GB Ascend NPU、310P/910B4 组合以及可调 vNPU，链路覆盖 1--100 Gbps 带宽和 1 us--20 ms 时延范围。模型切分实验需要单独覆盖异构拓扑，而不是只在同构多卡环境中验证。建议至少包含 A800T-A2 两机四卡 100 Gbps 直连、A800T-A2 两机四卡 25 Gbps host RDMA、单机两卡 56 GBps HCCS、半算力或四分之一算力卡，以及 310P--910B4 联合组网等配置。每组实验都应记录 layer-level profile、stage 耗时、stage 间 activation/hidden state 传输、pipeline bubble、显存峰值和回退次数。这样才能判断收益来自合理的层级切分和拓扑放置，而不是来自单纯增加副本数或换用更强设备。

用户移动场景应单独作为重点实验，而不是混在普通多节点实验中。SOW 中的关键设定是三个同模型推理实例、三个请求客户端、默认就近实例、prefix cache 开启；测试运行超过 50\% 后，将 20\% 请求发送到另外两个实例，以模拟入口变化和历史状态位置分离。该场景需要记录入口变化、服务实例变化、状态访问路径变化和 token streaming 中断情况。这样才能判断 mobility-aware routing 是否真的改善了服务连续性，而不是只是在平均时延上略有收益。
...
指标体系应分为服务体验、模型切分质量、系统效率、网络状态和质量约束五层。服务体验层关注用户直接感知到的指标，包括平均 E2E 时延、P90/P99 TTFT、TPOT、ITL、SLA 达标率、请求失败率和 token streaming 中断时间。对于交互式 LLM 服务，平均时延不够，P90/P99 TTFT 和 ITL 更能反映尾部体验；对于工具调用或工作流型请求，deadline miss ratio 比单纯 TPOT 更重要。
...
质量与约束层则用于防止系统优化以牺牲生成效果、隐私或公平性为代价。应记录任务成功率、生成质量代理指标、模型降级/回退比例、隐私敏感请求本地处理比例、不同 SLO 等级用户的违约率差异，以及移动用户与静态用户之间的体验差异。如果只保留核心验收指标，建议优先选择 P90/P99 TTFT、TPOT/ITL、平均 E2E latency、SLA 达标率、满足 SLA 的 rps 或用户数、GPU/NPU 利用率、prefix cache 命中率、KV/state 迁移成本和 handover 中断时间。这组指标既能体现 LLM serving 性能，也能体现无线 CEC 的场景价值。
...
预期收益应围绕三项算法交付分别验证，而不是按研发远近粗略拆分。模型切分部署算法的收益主要体现在异构资源利用和可支持并发规格上：通过以 layer-wise stage boundary 和 stage placement 为主轴的拓扑感知切分，先避免平均层级切分在异构算力、半算力卡、跨主机 RDMA 或 310P--910B4 联合组网中的 stage 不均衡和通信浪费；再按需叠加 TP/EP/DP 等补充能力，缓解单个 stage 的显存压力、MoE expert 驻留压力和流量承载压力。该算法的收益应主要通过相对多节点平均切分基线的满足 SLA 请求处理数、SLA-constrained concurrency、pipeline bubble ratio 和回退次数来体现，并支撑满足 SLA 请求处理能力提升 50\%、支持并发用户数达到 1.5X 基线的规格目标。

业务感知 batching 算法的收益主要体现在 SLA goodput 和平均 E2E 时延上。通过 SLO-aware admission、请求重排、动态等待窗口、batch/token budget 控制、chunked prefill 和 KV 水位保护，系统可以在随机并发、不同输入输出长度和 HTTP header 特殊时延要求下减少高优先级请求违约，降低长 prompt 对流式 decode 的干扰。验收时应相对于“静态 batching 配置、不调整请求处理顺序”基线，验证满足 SLA 的请求处理速度或 rps 提升 20\%，所有请求平均 E2E latency 降低 20\%。

移动请求调度算法的收益主要体现在多实例协同和服务连续性上。它不一定显著提高单实例峰值吞吐，但可以在多流量入口、不同链路距离和用户入口切换条件下减少不必要的就近拥塞，降低移动用户服务中断和重复 prefill，提升 prefix cache 或 session state 的利用率，并把请求分散到更合适的边缘实例。验收时应相对于“就近实例路由”基线，验证满足 SLA 的用户数达到 1.5X，所有请求平均 E2E latency 降低 20\%。对于无线业务，这类收益比单点吞吐更贴近用户感知，因为真实体验受路径稳定性、响应连续性和尾部时延共同影响。
