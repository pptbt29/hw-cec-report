...
移动请求调度以甲方交付配置为主：3 台 A800T-A2 部署相同模型并开启 prefix cache，两条链路为 100 Gbps 卡间直连，另一条为 25 Gbps 跨主机 RDMA；三个客户端使用不同数据集，前 50\% 时段请求发送至就近实例，后 50\% 时段将 20\% 请求改发至另外两个实例。除全程无移动对照和 10\%/20\%/30\% 改发比例外，不再扩展复杂移动轨迹，以保证结果可复现且便于交付。
...
移动请求调度算法的收益主要体现在多实例协同和服务连续性上。它通过减少不必要的就近拥塞、重复 prefill 和无效 KV 移动，提高 prefix/session state 利用率，并把请求分配到更合适的实例。交付验收以甲方参考 SLA 和并发数为基线，目标是在相同模型、数据集和硬件配置下，使满足 SLA 的并发数提高 50\%，平均 E2E latency 降低 20\%；P95 E2E、SLA violation 和 KV 传输量作为辅助解释指标。
