...
面向异构边缘验证环境，资源自适应模型切分算法需要形成可运行的部署规划、测试结果和可解释分析 \refcite{76}。因此，第四章的算法建议不能把模型切分简单放在“后续研究”中，也不宜被写成从零实现一个任意切 layer、切 tensor、迁移 KV 的新 runtime。更合理的定位是：在 Ascend/CANN/MindIE 或 vLLM-Ascend 等现有推理栈上实现一个\textbf{以 Layer-wise / Vertical Partitioning 为主轴的资源自适应切分部署规划器}，先决定 Transformer layer、block 或 model shard 如何跨节点形成 pipeline stage，再根据单个 stage 的显存和算力压力叠加 TP；对于 MoE 模型再引入 EP；DP 则主要用于实例副本扩展和流量承载，而不是模型本体切分的核心 \refcite{77}, \refcite{79}, \refcite{82}。

从输入信息看，该规划器需要读取三类画像。第一类是硬件拓扑画像，包括算力节点可用算力、可用显存、vNPU 配置、节点间带宽和时延；验证环境覆盖 ARM CPU、230T/64GB Ascend NPU、70T/24GB Ascend NPU、A800T-A2 多机多卡、HCCS、25Gbps RDMA、100Gbps 直连以及 310P 与 910B4 联合组网等异构条件 \refcite{76}。第二类是模型画像，包括 CodeLlama34B、Qwen2-VL-7B-Instruct 等模型的参数量、每层权重、activation、单 token KV Cache、prefill/decode 计算量和显存需求。第三类是业务画像，包括并发用户数、SLA/P90/P99 TTFT 目标、输入输出长度分布和请求优先级。只有把这三类信息放入同一个约束模型，模型切分才不只是论文 taxonomy，而是可交付算法。

推荐的算法主体可以分为三个模块。第一个模块是层级切分候选生成。系统应先从 layer/block 边界出发，枚举可部署的 pipeline stage 数量、每个 stage 覆盖的连续层范围以及 stage 到 Ascend 节点的 placement；在此基础上，再生成 TP、EP、DP、context parallel、prefill/decode disaggregation 和 KV Cache Pool 等补充模板。不支持或无法稳定部署的模板直接过滤。这样做的意义不是降低算法要求，而是把搜索空间限制在验证环境可执行的方案上，同时保证模型切分算法的核心仍然落在 layer-wise placement 上。
...
因此，第四章面向验证环境最适合作为主线的不是单纯的“部署模板选择”，也不是泛泛的 DP/TP/PP/EP 参数枚举，而是\textbf{以层级切分为主轴、TP/EP/DP 为补充的拓扑感知混合切分部署算法}。模板库和 profiling 是算法的工程基础，layer-wise stage boundary 与 placement 是主要决策对象，在线选择和受控重配置是算法的运行形态；三者共同服务于模型切分交付目标。这样的定位既保留论文综述中的 tensor/operator-level、expert/component-level、phase-level 等方法视野，也能把第四章的主问题稳定落在“模型如何沿深度方向跨端-边-云展开，并在必要时叠加更细粒度并行”上。
\phantomsection
\subsection*{4.7 小结}
