...
本项目包含三个相同模型的推理节点，每个节点同时作为请求入口和可选计算节点，并开启 prefix cache。其中两条连接为 100 Gbps 高速链路互联，另外一条为 25 Gbps 跨主机 RDMA 链路。测试前半段，请求主要由对应入口节点处理；测试进行到总时长的 50\% 后，部分请求从其他节点接入，以模拟用户移动后请求入口与历史 KV cache 所在节点发生分离的场景。系统需要在满足 P99 TTFT SLA 和 GPU memory 约束的前提下，选择请求的计算节点及其 KV cache 获取方式，并最小化整个 session 的长期服务成本。

总体思路是把一次请求卸载看作“计算节点选择 + 状态处理方式选择”的联合决策，而不是只根据入口节点或当前最短时延做就近执行。对于没有历史状态的请求，系统主要在多个可用节点之间选择执行位置；对于已有多轮上下文或 KV cache 的 session，系统还需要判断是在状态所在节点继续执行、将状态迁移到新的计算节点，还是在目标节点重新计算必要状态。这样，请求路由同时感知当前 SLA、显存容量、节点负载、链路差异和后续 session 成本，使一次卸载决策既能满足当前请求可执行性，也尽量避免用户移动后长期跨节点访问状态带来的额外通信开销。

为支撑请求到达后的在线卸载决策，系统首先在后台持续运行分布式状态维护机制。三个节点分别部署本地路由器，后台负责同步节点状态、链路状态和 KV cache 元数据，并维护时延、显存和状态迁移成本的预测信息。节点在接收或完成请求、显存和 KV cache 占用变化、状态迁移完成以及链路状态变化时，向其他节点发送增量更新，同时通过周期性心跳校准全局状态。同步信息包括各节点的请求负载、预计排队时间、近期 TTFT、剩余显存、可用 KV cache 容量、节点间 RTT 与带宽，以及 session ID、prefix hash、KV block 位置、状态大小和版本。请求数据和 KV cache 直接在入口节点、状态源节点与目标计算节点之间传输，避免额外的数据中转。

在这些后台信息持续更新的基础上，当一个请求到达入口节点后，系统按如下步骤完成卸载决策与执行。
...
第三，构造最小时延 Greedy 基线。该基线仅考虑当前请求，选择预计端到端时延最低的动作：
...
第四，以长期累计成本作为完整路由策略的优化目标。本方法的重点不是单纯为当前请求寻找一个最快节点，而是围绕 KV cache 所在位置、用户后续入口位置和节点负载演化，决定当前是否值得迁移、重算或继续远程访问状态。换言之，策略的中心是学习“当前动作对未来 session 成本的影响”：一次迁移可能增加当前 TTFT，但如果后续多轮请求都会从新入口到达，就可能降低长期跨节点通信；一次重算可能浪费当前 prefill 计算，但可能避免在低带宽链路上传输大规模 KV cache。

该问题可以自然建模为带约束的 Markov decision process，并计划使用 reinforcement learning 学习长期路由策略。系统状态 \(s_t\) 包括当前请求入口、各节点负载和剩余显存、网络状态、用户移动趋势，以及 KV cache 的位置、大小和版本；动作是在可行动作集合中选择 local、migrate 或 recompute 方案；reward 或 cost 同时反映当前请求时延、状态传输开销、显存压力、SLA violation 惩罚和后续状态价值。对于每个可行动作，其长期价值定义为：
...
该 RL 问题的难点在于状态、约束和反馈都具有较强复杂性。首先，状态空间同时包含离散的 KV owner、连续的负载和显存水位、动态变化的链路质量，以及难以准确预测的用户移动趋势。其次，LLM 请求的输出长度、decode 时间和 KV 增长量在执行前并不确定，导致状态转移和即时成本存在明显随机性。再次，SLA 和 memory 是硬约束，不能依靠 RL 在线探索大量不可行动作，否则会直接造成 TTFT violation 或显存不足。最后，迁移或重算的收益往往体现在后续多轮请求中，reward 具有延迟性，短期看似更慢的动作可能长期更优。

因此，本项目不直接让 RL 在所有节点和状态操作上盲目探索，而采用“可行性过滤 + 低维动作选择 + 长期价值学习”的设计。前两步先用显式模型根据 SLA 和 memory 过滤不可行动作，保证进入 RL 策略的动作已经满足当前请求的基本可执行性；随后主方法使用 RL 学习长期状态价值，并只在少量可行动作中选择，使策略主要关注 local、migrate 和 recompute 之间的长期权衡。训练阶段可先基于真实 trace、移动模式和节点负载构建仿真环境，离线训练 value function 或 actor-critic 策略；在线运行时结合最新后台元数据进行推理。对于三节点项目规模，有限时域滚动规划主要作为辅助枚举和解释机制：系统预测未来若干个 session 请求的入口位置、节点负载和 KV 增长，对每一步重新执行 SLA 与 memory 可行性过滤，再用学习到的状态价值评估窗口末端成本，从而在可解释的枚举决策中引入长期学习能力。Greedy 最小时延策略则作为对比基线和在线安全回退，当预测信息不足或 RL 输出不稳定时保证请求仍可按当前可行的低时延方案执行。

Quality-of-Service Aware LLM Routing for Edge Computing with Multiple Experts \refcite{55} 将动态请求路由建模为长期 QoS 优化问题，说明路由决策需要考虑当前动作对后续系统状态的影响；MGCO \refcite{61} 将历史决策和未来移动状态纳入卸载过程，说明移动场景下不能只依据当前节点负载做短视选择。本文方案在这些思路基础上，将长期优化的核心进一步落到 KV cache 位置、状态迁移/重算成本和多轮 session 连续性上，使 RL 学习的对象不是抽象的节点选择，而是受 SLA、memory 和状态复用共同约束的请求卸载策略。
