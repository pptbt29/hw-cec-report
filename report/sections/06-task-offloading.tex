...
\begin{itemize}
    ...
    \item \textbf{用户移动性下的状态位置挑战。} 现有 CEC 移动任务卸载研究多面向普通计算任务，主要处理接入链路变化、任务迁移和结果回传路径。LLM 推理任务在移动场景下还需要处理历史状态的位置问题：当用户在多轮 session 中从一个基站覆盖区移动到另一个覆盖区时，后续 request 的入口节点发生变化，而历史 KV cache 可能仍保留在旧计算节点。系统因此需要同时决定当前计算节点和 KV 获取方式，即就地复用、跨节点同步或在目标节点重算。
\end{itemize}
...
本文采用“预测--过滤--长期决策”的在线调度框架。先用预测模型刻画移动性和输出长度不确定性，再用显式约束过滤不可行动作，最后在可行动作集合中进行长期价值比较：
\begin{itemize}
    \item \textbf{面向移动性的状态预测。} Mobility predictor 根据用户历史轨迹、当前入口节点和相邻 request 间隔预测未来入口分布，使调度器能够感知后续 request 可能访问历史状态的位置。
    \item \textbf{面向输出长度的资源预测。} Output length predictor 根据 prompt、上下文长度、任务类型和会话历史预测输出长度分布，使 decode 时间、返回流量、显存压力和新增 KV 规模在决策阶段提前进入估计。
    \item \textbf{面向 KV 状态的联合动作。} 调度器将“计算节点 + KV 获取方式”作为联合动作，其中 KV 获取方式包括 reuse、sync 和 recompute，从而把状态复用、状态同步和状态重算显式纳入卸载决策。
    \item \textbf{面向长期成本的安全决策。} 系统根据 E2E SLA、显存余量、节点可用性和 KV 版本一致性生成 action mask；Double DQN 只在可行动作集合中比较长期成本，从而同时考虑当前服务质量和后续 KV 复用代价。
\end{itemize}
...
\(\psi_k^{\mathrm{cache}},G,H\) & KV 副本保留/驱逐策略、KV 状态转移函数和完整系统状态转移函数 \\
...
\(C_k^{\mathrm{eviction}},w_1,\ldots,w_4,c_k\) & 跨 session KV 驱逐外部性、成本权重和当前期望成本 \\
...
\caption{\textbf{单个请求到达后的长期成本感知卸载决策流程。}Request 被拆分为用户信息、prompt 信息和 SLA 信息：用户信息进入 mobility predictor 生成未来入口分布，prompt 信息进入 output length predictor 估计 decode 长度，CEC system state 与 KV context manager 分别提供节点负载、链路带宽、剩余显存以及 KV block 位置/版本等运行状态。State builder 将移动预测、长度预测、SLA、系统遥测和 KV 状态汇聚为 feature bundle；action enumerator 枚举“计算节点 + KV 获取方式”候选，per-action estimator 对每个候选动作估计请求传输、KV 同步/重算、prefill、decode 和显存占用。E2E SLA filter 与 Memory/KV filter 先处理时延、显存和 KV 可用性等硬约束，得到 feasible actions 作为 DQN action mask；Double DQN router 只在可行动作中比较长期成本并输出 final action。环境执行后的实际 E2E 时延、输出长度、KV 增量、传输量和显存变化写入 replay buffer，用于回传更新预测器、估计器和路由策略。}
...
\]
固定请求间隔时可采用 Markov chain，间隔变化较大时可采用 continuous-time 或 semi-Markov 模型；LSTM 或 Transformer 则输出多步入口分布 \(b_k\)。这一设计借鉴 MGCO 对历史决策和未来移动状态的利用 \cite{ghosh2025mgco}。输出 token 数 \(Y_k\) 在决策时未知，由预测器给出 \(\rho_k(y)=\Pr(Y_k=y\mid x_k)\)；类似的长度预测已用于不可预测请求调度 \cite{li2025cloud}。长度预测不仅用于当前请求的 E2E latency 和显存需求估计，也进入动作价值评估：长输出会增加 decode 时间、返回流量、KV 增量和后续显存压力，从而改变 DQN 对不同计算节点与 KV 获取方式的偏好。
...
\mathcal C_{k+1}
=
G(\mathcal C_k,a_k,Y_k,\Delta\mathcal B_k(a_k,Y_k),\psi_k^{\mathrm{cache}}).
\]
\(\mathrm{reuse}\) 仅在目标节点 \(J_k\) 已经持有当前请求所需的连续 KV prefix 或 blocks 时可行；若目标节点缺失必要 block，则该节点只能选择 \(\mathrm{sync}\) 或 \(\mathrm{recompute}\)。因此下一轮 KV 状态不仅取决于 cache policy，也取决于本轮选择的 \(a_k\)、真实输出长度 \(Y_k\) 以及由此产生的 KV 增量 \(\Delta\mathcal B_k(a_k,Y_k)\)。
...
\end{array}
\]
其中 \(c_k(s_k,a_k)\) 已对当前输出长度取条件期望。为避免公式过长，记
\[
s_{k+1}^{a,y,u'}
=
H(s_k,a_k,y,u',\Delta\mathcal B_k(a_k,y),\psi_k^{\mathrm{cache}})
\]
为在当前动作 \(a_k\)、实际输出长度 \(y\)、下一入口 \(u'\) 和 KV 增量共同作用后的下一状态。对应的理论最优状态--动作价值满足
\[
\begin{array}{rl}
\mathcal Q^*(s_k,a_k)
={}&c_k(s_k,a_k)\\
...
\end{array}
\]
移动性既影响当前转发成本，也通过 \(P_{U_ku'}\) 影响未来价值；输出长度不仅影响当前 decode 和显存，还通过 \(\Delta\mathcal B_k(a_k,Y_k)\) 改变后续 KV 分布，所以优化对象是整个 session，而非单次最低时延。这延续了 QoS-LLM-Router 的长期 QoS 优化 \cite{yang2025quality} 和 MGCO 的非短视移动卸载思想 \cite{ghosh2025mgco}，并将 CA-AIGC-Migration 的移动 context 管理 \cite{wang2026context} 细化到版本化 KV blocks。

上式定义了希望逼近的 \(\mathcal Q^*\)。对短 session、离散状态且已知真实轨迹的小规模实验，可用有限时域动态规划或搜索得到离线 oracle；但是一般场景中同一 session 的 request 数量 \(K\) 不确定，且存在动作序列和状态爆炸，因此采用 Dueling Double DQN 近似求解。DQN 对所有固定动作输出 Q 值，在执行 \(\arg\min\) 前将 \(a\notin\mathcal A_k^F(s_k)\) 的 Q 值置为 \(+\infty\)；
...
\end{array}
\]
即训练目标等于“当前成本 + 折扣后的观测下一状态价值”；若采用模型化 roll-out，也可用上式中的 \(\sum_y\rho_k(y)\sum_{u'}P_{U_ku'}\) 对下一状态取期望。在线动作则为 \(a_k^*=\arg\min_{a\in\mathcal A_k^F(s_k)}\mathcal Q_\theta(s_k,a)\)。若模型异常则回退至可行动作中的 Greedy 方案；若集合为空，则排队、降级或拒绝。

\subsubsection*{6.4.7 分布式 KV 管理}
\addcontentsline{toc}{subsubsection}{6.4.7 分布式 KV 管理}

借鉴 Mooncake 的 KVCache-centric 全局存储思想 \cite{qin2025mooncake}，系统建立分布式 block directory，记录 model/adapter 版本、prefix hash、token 区间、block 版本、大小和所在节点。执行 \(\mathrm{sync}\) 前，目标节点先查询已有 blocks，系统根据 \(\mathcal M_{k,J_k}\) 选择一个或多个源节点，并将缺失部分直接传至计算节点；这也吸收了 prefill/decode 分离系统中 KV handoff 的思路 \cite{zhong2024distserve}。同步不等于删除：源 blocks 继续保留，直至 cache policy 根据 TTL、显存压力、未来入口与复用概率、增量同步收益和重建成本决定驱逐。

KV manager 还可以在不扩大 RL 动作空间的情况下，把一次 \(\mathrm{sync}\) 细化为“部分迁移 + 部分重算”。在依赖关系允许时，系统只从其他节点迁移带宽开销较低或重算代价较高的缺失 blocks；其余缺失 blocks 则从目标节点已有的连续 prefix/checkpoint 开始重算。迁移与重算可按 block 流水并行，使计算 bubble 尽量覆盖通信 bubble。该机制不改变上层动作定义，仍表现为 \((J_k,\mathrm{sync})\)，但 KV manager 会用实际状态准备时间和实际迁移字节数替换基础全量同步估计。由此，Mooncake 的集群级 KV 管理被扩展为面向用户移动的版本化多副本与混合恢复管理。
