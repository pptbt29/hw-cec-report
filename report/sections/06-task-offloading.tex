...
为支撑请求到达后的在线卸载决策，系统首先在后台持续运行分布式状态维护机制。三个节点分别部署本地路由器，后台负责同步节点状态、链路状态和 KV cache 元数据，并维护时延、显存和状态迁移成本的预测信息。节点在接收或完成请求、显存和 KV cache 占用变化、状态迁移完成以及链路状态变化时，向其他节点发送增量更新，同时通过周期性心跳校准全局状态。同步信息包括各节点的请求负载、预计排队时间、近期 TTFT、剩余显存、可用 KV cache 容量、节点间 RTT 与带宽，以及 session ID、prefix hash、KV block 位置、状态大小和版本。请求数据和 KV cache 直接在入口节点、状态源节点与目标计算节点之间传输，避免额外的数据中转。在这些后台信息持续更新的基础上，当一个请求到达入口节点后，系统按如下步骤完成卸载决策与执行。
...
因此，本项目不直接让 RL 在所有节点和状态操作上盲目探索，而采用“可行性过滤 + 低维动作选择 + 长期价值学习”的设计。前两步先用显式模型根据 SLA 和 memory 过滤不可行动作，保证进入 RL 策略的动作已经满足当前请求的基本可执行性；随后 RL 只在少量可行动作中学习选择，使策略主要关注 local、migrate 和 recompute 之间的长期权衡。训练阶段可先基于真实 trace、移动模式和节点负载构建仿真环境，离线训练 value function 或 actor-critic 策略；在线运行时结合最新后台元数据进行推理，并保留 greedy 最小时延策略作为安全回退。对于三节点项目规模，还可以将 RL 价值函数与有限时域滚动规划结合：预测未来若干个 session 请求的入口位置、节点负载和 KV 增长，对每一步重新执行 SLA 与 memory 可行性过滤，再用学习到的状态价值评估窗口末端成本，从而在可解释的枚举决策中引入长期学习能力。
