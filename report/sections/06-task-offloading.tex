...
图~\ref{fig:ch6-mobility-offloading-workflow} 给出了单个 request 到达后的在线处理流程：系统从 request 中提取用户、prompt 和 SLA 信息，分别用于移动预测、输出长度预测和动作枚举；同时从 CEC 系统和 KV context manager 读取节点负载、链路带宽、显存水位与 KV block 状态。预测结果和系统状态先汇入 state builder，再由 per-action estimator 对每个候选动作估计通信、KV 同步/重算、prefill、decode 和显存开销；候选动作经过 E2E SLA 与 Memory/KV 过滤后形成 feasible actions，最终由 Double DQN router 在可行动作中选择长期价值最优动作，并通过执行反馈更新 replay buffer。
...
\textbf{动作掩码 Double DQN。} 上式定义了希望逼近的 \(\mathcal Q^*\)。对短 session、离散状态且已知真实轨迹的小规模实验，可用有限时域动态规划或搜索得到离线 oracle；但是一般场景中同一 session 的 request 数量 \(K\) 不确定，且存在动作序列和状态爆炸，因此采用 Dueling Double DQN 近似求解。DQN 对所有固定动作输出 Q 值，在执行 \(\arg\min\) 前将 \(a\notin\mathcal A_k^F(s_k)\) 的 Q 值置为 \(+\infty\)；
