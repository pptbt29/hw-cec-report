...
\(U_k,J_k\) & 当前请求入口节点和所选计算节点 \\
\(Y_k,\rho_k(y)\) & 实际输出 token 数及其预测分布 \\
\(P_{ij}(\Delta\tau_k),b_k\) & 入口转移概率和多步移动预测分布 \\
\(\mathcal C_k\) & 各节点上 KV blocks 的位置与版本状态 \\
\(\mathcal B_{k,n},\mathcal M_{k,j}\) & 节点 \(n\) 已有 blocks 和节点 \(j\) 缺失 blocks \\
\(a_k=(J_k,Z_k)\) & 计算节点与 KV 获取方式组成的联合动作 \\
\(\mathcal A_k^F\) & 满足 SLA、显存、版本和节点可用性约束的动作集合 \\
...
\(D_k^{\mathrm{request}},D_k^{\mathrm{KV}},C_k^{\mathrm{eviction}}\) & 请求字节数、KV 同步字节数和 KV 驱逐损失 \\
\(T_k^{\mathrm{E2E}},c_k\) & 当前请求端到端时延和即时成本 \\
\(\mathcal Q^*,\mathcal Q_\theta,\mathcal Q_{\bar\theta}\) & 最优状态--动作价值、Double DQN 在线网络和目标网络 \\
\(\gamma\) & 未来成本折扣因子 \\
\bottomrule
\end{tabularx}
\end{center}
...
\]
固定请求间隔时可采用 Markov chain，间隔变化较大时可采用 continuous-time 或 semi-Markov 模型；LSTM 或 Transformer 则输出多步入口分布 \(b_k\)。这一设计借鉴 MGCO 对历史决策和未来移动状态的利用 \refcite{61}。输出 token 数 \(Y_k\) 在决策时未知，由预测器给出 \(\rho_k(y)=\Pr(Y_k=y\mid x_k)\) ；类似的长度预测已用于不可预测请求调度 \refcite{56}。若下一请求在当前回复结束后产生，则 \(\Delta\tau_k(y)=T_k^{\mathrm{E2E}}(y)+T_k^{\mathrm{think}}\)，生成长度也会影响下一入口概率。
...
\mathcal C_{k+1}
=
G(\mathcal C_k,a_k,\Delta\mathcal B_k(Y_k),\psi_k^{\mathrm{cache}}).
\]
...
&Y_k\sim\rho_k,\quad
U_{k+1}\sim P(\cdot\mid U_k,\Delta\tau_k(Y_k)),\\
&\mathcal C_{k+1}
=G(\mathcal C_k,a_k,\Delta\mathcal B_k(Y_k),\psi_k^{\mathrm{cache}}).
\end{array}
\]
其中 \(c_k(s_k,a_k)\) 已对当前输出长度取条件期望，而 \(Y_k\) 仍通过 KV 增长、请求间隔和下一状态影响未来成本。对应的理论最优状态--动作价值满足
\[
\begin{array}{rl}
\mathcal Q^*(s_k,a_k)
={}&c_k(s_k,a_k)\\
...
+\gamma\sum_y\rho_k(y)
\sum_{u'\in\mathcal N}
P_{U_ku'}(\Delta\tau_k(y))
\min_{a'\in\mathcal A_{k+1}^F(s_{k+1}(y,u'))}
\mathcal Q^*(s_{k+1}(y,u'),a').
\end{array}
\]
终端条件为 \(\mathcal Q^*(s_{K+1},\cdot)=0\)。式中当前期望成本 \(c_k\) 位于外层，未来价值才对输出长度 \(y\) 和下一入口 \(u'\) 分别取期望，因而括号不存在跨行配对问题。移动性既影响当前转发成本，也通过 \(P_{U_ku'}\) 影响未来价值；当前动作改变 KV 分布，所以优化对象是整个 session，而非单次最低时延。这延续了 QoS-LLM-Router 的长期 QoS 优化 \refcite{55} 和 MGCO 的非短视移动卸载思想 \refcite{61}，并将 CA-AIGC-Migration 的移动 context 管理 \refcite{69} 细化到版本化 KV blocks。

\textbf{动作掩码 Double DQN。} 上式定义了希望逼近的 \(\mathcal Q^*\)。对短 session、离散状态且已知真实轨迹的小规模实验，可用有限时域动态规划或搜索得到离线 oracle；一般场景存在动作序列和状态爆炸，因此采用 Dueling Double DQN 近似求解。DQN 对所有固定动作输出 Q 值，在执行 \(\arg\min\) 前将 \(a\notin\mathcal A_k^F\) 的 Q 值置为 \(+\infty\)；训练和在线决策使用同一 mask，从而避免选择显存、版本或节点条件不满足的动作。

令分支 \(\omega=(y,u')\) 表示“输出长度为 \(y\)，下一请求从 \(u'\) 进入”，其概率为
\[
p_k(\omega)=\rho_k(y)P_{U_ku'}(\Delta\tau_k(y)).
\]
Double DQN 先用在线网络选择动作，再用目标网络评价：
\[
\begin{array}{rl}
a_{k+1}^{\mathrm{sel}}(\omega)
={}&\displaystyle\arg\min_{a'\in\mathcal A_{k+1}^F}
\mathcal Q_\theta(s_{k+1}(\omega),a'),\\
V_{k+1}(\omega)
={}&\mathcal Q_{\bar\theta}
(s_{k+1}(\omega),a_{k+1}^{\mathrm{sel}}(\omega)),\\
z_k
={}&c_k(s_k,a_k)
+\gamma\displaystyle\sum_{\omega}p_k(\omega)V_{k+1}(\omega).
\end{array}
\]
...
甲方交付实验采用固定主场景：部署 3 台 A800T-A2，其中两条链路为 100 Gbps 卡间直连，另一条为 25 Gbps 跨主机 RDMA；三个节点运行相同模型并开启 prefix cache，三个客户端使用不同测试数据集。测试前 50\% 时段，请求发送至就近实例；后 50\% 时段，将 20\% 请求随机改发至另外两个实例，以模拟用户移动。SLA 和初始并发采用甲方给定参考值，验收目标是在该基线之上将满足 SLA 的并发数提高 50\%，并将平均 E2E latency 降低 20\%。

为控制实验复杂度，主对比仅设置就近路由、当前成本 Greedy 和本文 RL 三种策略；补充实验只增加“全程无移动”对照，以及 10\%/20\%/30\% 改发比例的敏感性分析。核心指标为 SLA 下最大并发数、平均/P95 E2E latency 和 SLA violation；KV 传输量、prefix/block 命中率、同步/重算次数与显存峰值仅用于解释结果。
