...
LLM serving 中的 batching 不再只是传统 DNN 推理里的固定 batch size 选择。一次 LLM 请求通常先经历 prefill，再进入逐 token decode；请求到达时间、prompt 长度、输出长度、SLO、KV Cache 占用和上下文复用机会都会随时间变化。因此，batching 的核心问题可以定义为：在给定请求队列、实例负载、KV 状态和端-边-云链路条件下，动态决定哪些请求进入 batch、batch 在什么粒度更新、prefill 与 decode 是否混合、长 prompt 是否切块、不同阶段是否解耦，从而在 TTFT、ITL、TPOT、E2E latency、throughput、goodput 和资源利用率之间取得平衡。Orca 将生成式 Transformer serving 的调度粒度推进到 iteration level \cite{yu2022orca}；vLLM 通过 PagedAttention 为 continuous batching 提供了 KV 内存管理基础 \cite{kwon2023efficient}；Sarathi-Serve 代表 chunked prefill 路线 \cite{agrawal2024taming}，DistServe 则代表 prefill/decode disaggregation 路线 \cite{zhong2024distserve}。
...
这四类不是严格互斥的，而是从 batching 形成与更新粒度描述不同批处理组织方式。传统 Static Batching 仍可作为 baseline，但不再作为本章一级方法，因为它主要代表固定批次假设，并不能体现 CEC 下的业务感知和在线自适应。Queue-level dynamic/admission batching 可以叠加 SLO-aware priority；Continuous batching 可以叠加抢占、KV-aware admission 和 active set 调整，Orca 和 FastServe 分别体现了 iteration-level scheduling 与 preemptive scheduling 的代表路线 \cite{yu2022orca}, \cite{wu2026fastserve}；Chunked Prefill / Blended Batching 仍然可能运行在 continuous batching runtime 上，Sarathi-Serve 是这一方向的典型系统 \cite{agrawal2024taming}；Disaggregated Prefill-Decode Batching 则进一步把两个阶段的 batch 从同一实例内部拆到不同资源池，DistServe 是阶段解耦路线的代表 \cite{zhong2024distserve}。这个分类的目的不是把每篇论文强行放入唯一格子，而是让读者看清：一个方法主要是在请求接纳/动态组批、迭代级 active batch、prefill chunk 混合，还是阶段解耦上做优化。
...
\tabrowrule
3 & QLM \cite{jhaone} & AIOps@ASPLOS 2024 & Queue-level Dynamic / Admission Batching & Queue management & 以满足端到端延迟 SLO 为目标，通过队列控制减少 head-of-line blocking 并提高 SLO attainment \\
\tabrowrule
...
多租户 LoRA serving 也可以从 continuous batching 角度理解。Punica 和 S-LoRA 的目标不是改变自回归生成逻辑，而是让请求不同 adapter 的序列仍能共享同一个基础模型执行批次。为此，它们需要解决 adapter 权重分页、异构 adapter batching 和定制 LoRA 算子问题；这些机制改变的是 batch 内请求能否被合并执行，以及合并后是否仍能保持较低的显存和算子开销 \cite{chen2024punica}, \cite{sheng2024slora}。
...
Chunked Prefill / Blended Batching 要解决的是长 prefill 阻塞 decode 的问题。Prefill 处理完整 prompt，矩阵规模大、并行度高，通常更偏 compute-bound；decode 逐 token 生成，频繁访问 KV Cache，更偏 memory-bound 或 latency-bound。如果长 prompt prefill 连续占用 GPU，已经在流式输出的请求会出现 ITL 抖动；如果完全优先 decode，新请求 TTFT 又会被推迟。两阶段混部时，关键问题不是是否 batching，而是 prefill 和 decode 如何在同一执行资源上交错。Sarathi-Serve 将长 prefill 切成 chunks 并与 decode 交错，是这一类方法的代表性工作 \cite{agrawal2024taming}。
...
面向华为验证环境，batching 参数优化需要在请求持续到达、prompt 长度变化、KV 水位变化和业务 SLO 不同的情况下，在线决定下一轮接纳哪些请求、按什么顺序执行、给 prefill 和 decode 分配多少 token 预算，以及是否启用 chunked prefill、长请求隔离或 PD 分离。该决策必须兼顾吞吐、尾延迟、公平性和显存安全，避免高优先级请求超时、普通请求长期饥饿或 KV Cache 触发 OOM。

因此，本章建议采用分阶段增强的轻量 actor-critic 强化学习在线 batching 优化器。算法输入包括推理实例集合 \(\mathcal{S}\)、等待请求集合 \(\mathcal{Q}_t\)、当前 active set、KV Cache 状态、硬件/模型画像和业务 SLO 配置；算法输出为当前控制窗口的 batching 动作。该问题的决策变量是控制窗口 \(t\) 内的动作向量
\[
a_t=(b_t,u_t,w_t,c_t,\rho_t,\alpha_t,\beta_t,\eta_t).
\]
其中，\(b_t\) 表示 max running requests 或目标 active batch 规模；\(u_t\) 表示 max batched tokens 或 token budget；\(w_t\) 表示最大等待窗口；\(c_t\) 表示 chunk size；\(\rho_t\) 表示 prefill/decode token quota；\(\alpha_t\) 表示 admission threshold；\(\beta_t\) 表示请求排序权重，用于综合 SLO slack、等待时间、业务优先级、预估 prefill 代价和 KV 增长；\(\eta_t\) 表示离散控制开关，包括长请求隔离、chunked prefill 以及在具备稳定 KV handoff 时可选的 PD 分离。actor 网络用策略 \(\pi_\theta(a_t\mid s_t)\) 根据当前状态 \(s_t\) 生成动作 \(a_t\)，其中 \(\theta\) 为策略参数，\(\pi_\theta(a_t\mid s_t)\) 表示在状态 \(s_t\) 下选择动作 \(a_t\) 的策略分布或打分函数；critic 网络估计该动作对 SLA 达标率、端到端时延、ITL 稳定性和 KV 风险的长期收益。算法核心是在 runtime 可执行动作空间中学习“接纳谁、按什么顺序执行、用多大 token/batch 预算、是否切 chunk、是否启用阶段控制”。

这些动作变量也不是在无限连续空间中任意生成。系统先根据 runtime 能力和当前资源状态构造可执行动作空间：由剩余 KV block、显存水位和实例并发上限给出 \(b_t\) 与 \(u_t\) 的取值范围；由业务 TTFT、deadline slack 和当前排队长度给出 \(w_t\) 与 \(\alpha_t\) 的范围；由长 prompt 比例、prefill/decode 时间占比和 ITL 抖动给出 \(c_t\) 与 \(\rho_t\) 的候选值；由请求业务等级、等待时间、预估服务时间和 KV 增量生成排序权重 \(\beta_t\) 的候选组合；由 runtime 是否支持 chunked prefill、长请求隔离和 PD/KV handoff 决定 \(\eta_t\) 中哪些开关可以打开。在此基础上，actor 只负责从可执行动作空间中选择或微调动作，安全层再根据 KV 上限、P99 TTFT 预测、OOM 风险和切换幅度对 \(a_t\) 做裁剪或回退。因此，第五章的强化学习不是让模型凭空发明 batching 参数，而是在业务和资源约束限定的动作空间内学习更合适的参数、排序和阶段控制组合。图~\ref{fig:ch5-batching-architecture} 给出了该 actor-critic 控制器的状态观测、动作生成、安全过滤和策略输出链路。

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figures/ch5_rl_batching_architecture.png}
\caption{基于 actor-critic 强化学习的在线 batching 优化架构}
\label{fig:ch5-batching-architecture}
\end{figure}

在线 batching 优化器不是简单输出一个固定 batch size，而是根据请求队列、KV 水位、active batch、prefill/decode backlog 和 SLO slack 生成一组可执行控制量，包括 batch/token 预算、等待窗口、准入阈值、排序权重、chunk size 和 prefill/decode quota。在论文建模上，可以先把每个控制窗口的决策写成带安全约束的动作选择问题：
\[
\begin{array}{ll}
\max\limits_{a_t\in \mathcal{A}_{\mathrm{safe}}(s_t)} & R_t(a_t) \\
\mathrm{s.t.}
& K_t(a_t)\leq K_{\max},\\
& \widehat T_{\mathrm{ttft}}(r_i,a_t)\leq D_i,\quad \forall r_i\in \mathcal{A}_t,\\
& a_t \in \mathcal{A}_{\mathrm{exec}}(s_t),\\
& \Delta(a_t,a_{t-1})\leq C_{\mathrm{switch}}.
\end{array}
\]
其中 \(R_t(a_t)\) 是动作在当前窗口带来的综合收益，\(\mathcal{A}_{\mathrm{safe}}(s_t)\) 是经过安全过滤后的动作集合，\(K_t\) 是动作执行后的 KV block 或显存占用，\(D_i\) 是请求 \(r_i\) 的 deadline 或 TTFT 约束，\(\mathcal{A}_{\mathrm{exec}}\) 表示当前 runtime 可执行动作集合，\(\Delta(a_t,a_{t-1})\) 限制相邻窗口的参数变化幅度。actor-critic 不需要在公式中展开复杂的长期期望项；可以理解为 actor 根据 \(s_t\) 产生动作，critic 根据当前收益和后续窗口反馈估计动作的长期价值。验收目标直接对应收益设计：相对于“静态 batching 配置、不调整请求处理顺序”的基线，满足 SLA 的请求处理速度或 rps 提高 20\%，所有请求平均 E2E 时延下降 20\% \cite{huawei2026internal}。

状态 \(s_t\) 由三类画像构造。第一类是硬件与实例状态，包括 GPU/NPU 型号、可用显存、KV block 剩余量、active sequence 数、waiting queue 长度、prefill queue 长度、decode backlog、最近 P90/P99 TTFT、TPOT/ITL、prefix cache 命中率和链路状态。第二类是模型与 workload 画像，包括模型类型、上下文长度、单 token KV Cache 增量、prefill/decode 计算比例、prompt token 估计、输出长度估计、VLM 或长 prompt 比例。第三类是业务需求配置，包括请求业务等级、HTTP header 中的特殊时延要求、deadline、SLO slack、优先级比例、是否允许长请求隔离、是否允许 chunked prefill、是否允许 PD 分离和降级回退。

可选动作的生成需要体现不同 batching 能力的适用条件。默认动作先保留框架现有 continuous batching 能力，只在安全范围内调整 max running requests、max batched tokens 和等待窗口；当长 prompt 或 VLM 请求比例升高，并且 prefill 对 decode ITL 的干扰明显时，再生成 chunked prefill、长请求隔离和 prefill/decode quota 调整动作；当存在稳定的 prefill pool、decode pool 和 KV handoff 观测时，才生成 PD 分离动作。图~\ref{fig:ch5-batching-architecture} 中 urgent batch、throughput batch 和 long-prompt chunks 表示策略输出会同时改变请求接纳、排序和 token 预算，而不是只调一个全局参数。请求排序和准入动作同时考虑 slack、请求 age、预估 prefill 代价、KV 增长和对已有 decode 流的干扰，使优先级只是排序信号之一，而不是唯一出队规则。

是否采用某个动作由安全过滤和强化学习策略收益共同决定。动作执行前先检查 KV 水位、OOM 风险、P99 TTFT 预测、runtime 支持能力和参数切换幅度；不满足约束的动作被裁剪或丢弃。通过过滤后，轻量 actor-critic 根据实际反馈更新策略，奖励可写为：
\[
R_t=
b_1 R_{\mathrm{sla}}
-b_2 L_{\mathrm{e2e}}
...
-b_4 M_{\mathrm{kv}}
-b_5 R_{\mathrm{oom}}
-b_6 F_{\mathrm{starve}}.
\]
其中 \(R_{\mathrm{sla}}\) 是满足 SLA 的请求处理速度或 rps，\(M_{\mathrm{kv}}\) 是 KV 水位压力，\(R_{\mathrm{oom}}\) 是 OOM 或回退风险。若某个动作提高了吞吐但导致高优先级请求 TTFT 违约、普通请求饥饿或 ITL 抖动恶化，奖励会下降，策略后续应减少选择该动作。

算法验证可采用三阶段递进方式。第一阶段只学习 batch 参数自适应，保持 FIFO 或框架默认排序不变，用 shadow mode 对比固定参数和在线参数策略，确认收益是否来自 max running requests、max batched tokens、等待窗口和 chunk size 的调整。第二阶段加入请求排序和准入控制，对 10\% 高优先级、10\% 低优先级和 80\% 普通请求的 workload \cite{huawei2026internal}，比较 FIFO、EDF/slack、priority queue 与策略学习到的排序权重，观察高优先级违约率、普通请求饥饿比例、P99 TTFT 和 ITL 抖动。第三阶段再形成混合策略，允许策略同时调节参数、排序权重、chunked prefill、长请求隔离和可选 PD 分离动作，重点观察满足 SLA 的请求处理速度、平均 E2E 时延、策略切换次数、回退次数和链路扰动下的稳定性。

最终的落地策略可概括为“先参数、再排序、后混合”。先学习 batch 参数自适应，保证在不改变请求顺序的情况下相对静态配置有稳定收益；再学习 SLO/代价感知的请求排序和准入权重；最后把参数、排序和阶段控制放入同一个受安全约束的在线策略学习闭环。Continuous batching、dynamic batch、chunked prefill 和 PD 分离只是底层可用能力，真正需要交付的是可观测、可回退、可解释的业务感知 batching 控制器。
\phantomsection
\subsection*{5.6 小结}
...
面向验证环境，第五章的主线应是分阶段增强的轻量 actor-critic 强化学习在线 batching 优化器：先学习 batch 参数自适应，再学习请求排序和准入权重，最后学习参数、排序和阶段控制的联合策略。这样的安排既能对应静态 batching/FIFO 基线，也能把算法收益落实到满足 SLA 的请求处理速度、平均 E2E 时延、TTFT/ITL 稳定性、高优先级请求违约率、策略切换次数和回退次数等可测指标上。
