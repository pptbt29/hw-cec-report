...
项目资料对 batching 参数优化算法的要求比普通 serving 调参更具体：系统需要感知推理实例硬件环境、模型特征、当前并发请求数、输入输出长度变化，以及每个请求可能携带的推理时延要求；算法需要动态调整请求处理顺序和推理实例批处理大小等参数，并在不中断业务的条件下对接推理框架实施。验收目标也明确指向可测收益：相对于“静态 batching 配置、不调整请求处理顺序”的基线，满足 SLA 的请求处理速度提高 20\%，所有请求平均 E2E 时延下降 20\% \refcite{76}。因此，第五章的算法建议应围绕一个可交付的\textbf{业务感知动态 batching 控制器}展开，而不是只描述 runtime 内部已有的 continuous batching 能力。

该控制器的第一部分是 SLO-aware admission and ordering。请求入口读取 HTTP header 中的特殊时延要求，将请求划分为高优先级、普通、低优先级三类；项目资料给出的 workload 中，10\% 请求携带 30--50ms 高优先级要求，10\% 请求携带 30--50ms 低优先级要求，其余为正常请求 \refcite{76}。算法需要为每个请求估计 prompt 长度、输出长度、已等待时间、剩余 slack 和预计服务时间，再计算动态调度分数。高优先级请求应该更快进入执行，但不能永久压制普通和低优先级请求；因此需要引入 aging、公平性下限和 deadline miss 风险保护，使调度结果既能降低高优先级违约率，也不会制造新的饥饿问题。
...
资源自适应模型部署的落点应从“动态任意切分模型”收敛为“拓扑感知的并行组合规划”。在现有 Ascend 栈中，候选方案可以包括单实例部署、DP、TP、PP、EP、layer sharding、context parallel、prefill/decode disaggregation、KV Cache Pool 和 speculative decoding 等。系统先离线压测这些方案在不同模型、上下文长度和并发下的性能，再由在线策略选择可部署的并行组合或实例池。对于第 4 章讨论的 layer-wise、operator-level、expert/component-level partitioning，应按框架能力分级处理：框架已经支持的 TP/PP/EP/CP 进入主线算法；需要轻量接口适配的 layer sharding、PD/KV 能力进入扩展验证；需要深度改造的在线 operator split 或跨实例 KV migration 则作为风险项单独评估。

\phantomsection
\subsection*{8.4 与项目周期对齐的实施路线}
\addcontentsline{toc}{subsection}{8.4 与项目周期对齐的实施路线}

项目资料给出的周期为 12 个月，T 到 T+2 月完成三类算法的技术调研和研究综述；T+3 到 T+8 月完成资源自适应模型切分算法与业务感知 batching 算法的设计、实现、测试验证和分析报告；T+9 到 T+12 月完成用户移动场景下请求调度算法的设计、实现、测试验证和分析报告 \refcite{76}。因此，工程路线应围绕这三个交付窗口组织，而不是用模糊时间标签替代真实排期。

T 到 T+2 月的重点是需求澄清、技术选型和验证环境准备。综述报告需要完成主流技术方案对比，明确三类算法的系统边界、可复用框架能力和需要适配的接口；同时应尽早跑通 vLLM Ascend 或 MindIE 的最小 serving baseline，确认 CANN、torch-npu、模型权重、tokenizer、容器镜像、NPU 设备映射和基础压测工具能够使用。这个动作服务于后续算法实现，不应被理解为独立算法交付。

T+3 到 T+8 月是模型切分与 batching 两项算法的主交付窗口。模型切分侧应完成硬件拓扑图构建、模型画像解析、DP/TP/PP/EP 候选方案生成、显存/链路/SLA 约束检查、拓扑感知搜索和部署执行接口；batching 侧应完成 SLO-aware admission、请求重排、动态 batch 参数控制、长 prompt 处理和必要的 chunked prefill 或阶段配额策略。两项算法可以共享 profiling 模块和指标采集模块：模型切分给出实例/模板能力边界，batching 根据这些边界动态调度请求。该窗口的测试应直接对齐 1.5X 并发用户规格、满足 SLA 的请求处理数提升 50\%、满足 SLA 的 rps 提高 20\% 和平均 E2E 时延下降 20\% 等目标 \refcite{76}。

T+9 到 T+12 月是移动请求调度算法的主交付窗口。此时前一阶段形成的实例画像、batching 行为和模型部署模板应作为 routing 的输入，而不是重新开始构建系统。策略网关需要部署在不同流量入口，读取入口到实例的带宽/时延、实例负载、模型能力、prefix cache 和 session 元数据，并在用户入口变化时选择合适推理实例。验证时应复现项目资料中的三实例场景：A800T-A2 三台机器、不同链路带宽、三个请求客户端、测试时长过半后部分请求切换入口，以比较就近接入基线和新调度算法在整体 SLA 用户规格与平均 E2E 时延上的差异 \refcite{76}。

按照这个周期，报告中的算法建议应形成“可直接交付、需要轻量适配、需要深度改造”三档。业务感知 admission、请求重排、batch 参数控制、拓扑感知 DP/TP/PP/EP 选择和多实例 routing 属于可直接交付或轻量适配范围；PD disaggregation、KV Cache Pool、prefix-aware routing 和 chunked prefill 属于取决于框架版本的扩展验证；在线 operator-level split、跨实例真实 KV migration 和复杂 semantic/token-level 协同则属于高风险能力，应在算法设计中保留接口和评估方法，但不宜作为达成主要验收指标的唯一依赖。

\phantomsection
\subsection*{8.5 风险、边界与建议}
\addcontentsline{toc}{subsection}{8.5 风险、边界与建议}

第一类风险是框架能力与论文假设之间的差距。论文中的动态模型切分、KV migration、state-aware routing 或 speculative verification 往往默认可以自由访问 runtime 内部状态；但在实际系统中，vLLM Ascend/MindIE 暴露的接口可能只支持一部分参数和指标。因此，报告后续应把算法分为“可直接交付”“需要轻量适配”“需要深度改造”三类。业务感知 admission、请求重排、外部 routing、实例权重调整、拓扑感知模板/并行组合选择属于可直接交付或轻量适配；prefix 元数据索引、SLO-aware queue policy、chunked prefill 参数控制属于轻量适配；跨实例真实 KV 迁移、在线 operator split、深度 scheduler 改造则属于需要深度改造的风险项。
