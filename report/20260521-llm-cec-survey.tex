...
面向华为无线软件实现中的边缘 AI 验证环境，第四章讨论的模型切分部署方法不宜被理解为“从零实现一个可以任意切 layer、切 tensor、迁移 KV 的新推理 runtime”。更稳妥的工程路线，是把论文中的切分思想转化为现有 Ascend 推理栈之上的部署模板选择问题：底层由 vLLM Ascend、MindIE、CANN、torch-npu、ATB/MindIE Turbo 等组件承担模型加载、算子执行、显存管理和已有并行能力，上层算法负责根据节点画像、模型画像、链路画像和 SLA 目标选择合适的实例模板与服务路径 \refcite{77}, \refcite{79}, \refcite{82}。

从验证环境约束看，模型切分部署算法需要感知算力节点可用算力、可用显存、节点间带宽和时延，CodeLlama34B、Qwen2-VL-7B-Instruct 等模型的参数规模、单 token KV Cache、推理算力和内存需求，以及并发用户量、SLA、输入输出长度分布等业务配置 \refcite{76}。这些信息仍然是算法核心，但其使用方式应从“在线搜索任意切分点”调整为“离线 profiling 生成候选模板，在线选择和回退”。候选模板可以包括单实例部署、张量并行或细粒度 TP、layer sharding、context parallel、prefill/decode disaggregation、KV Cache Pool、边缘小模型加区域大模型兜底，以及 speculative draft/verify 等现有或可配置能力 \refcite{79}, \refcite{80}, \refcite{81}。
...
验证设计上，建议将模型切分部署实验拆成四组。第一组是框架基线：在给定 Ascend 异构节点和网络条件下，分别运行默认 vLLM Ascend/MindIE 配置，记录 P90/P99 TTFT、E2E latency、TPOT、显存峰值、OOM、NPU 利用率和可支持并发数。第二组是模板画像：对单实例、TP、layer sharding、context parallel、PD 分离、KV pool/offload 等模板进行离线 profiling，形成模型-模板-硬件三维性能表。第三组是策略选择：比较固定模板、平均分配、负载感知选择、拓扑感知选择和 SLA-aware 模板选择，观察满足 SLA 的 rps、最高并发用户数和回退次数。第四组是扰动测试：人为改变可用显存、链路 RTT、带宽、并发水平和输入长度分布，验证算法是否能避免不可部署模板，并在性能下降时给出可解释的回退选择。

因此，第四章对华为项目最适合落地的不是“任意动态模型切分器”，而是“面向 Ascend 推理栈的资源自适应部署模板选择算法”。它与验证环境中的硬件条件、模型画像、SLA 表和验收指标能够直接对齐，也能自然承接第八章提出的工程路线：先把 vLLM Ascend/MindIE 基线跑稳，再利用已有并行、阶段解耦、KV 管理和 speculative decoding 能力做策略增强，最后再评估是否需要更深层的 runtime 改造。
\phantomsection
\subsection*{4.7 小结}
...
验证目标对 batching 参数优化算法提出了比较明确的工程约束：系统需要感知推理实例硬件环境、模型特征、当前并发请求数、输入输出长度变化，以及每个请求可能携带的推理时延要求；算法需要动态调整请求处理顺序和推理实例批处理大小等参数，并且在不中断业务的条件下对接推理框架实施。评价目标也不是抽象吞吐提升，而是相对于“静态 batching 配置、不调整请求处理顺序”的基线，提升满足 SLA 的请求处理速度，并降低所有请求平均 E2E 时延 \refcite{76}。因此，第五章的落地重点应放在外层业务感知策略控制，而不是重写 vLLM Ascend 或 MindIE 内部 scheduler。
...
验证设计上，建议第五章对应的算法实验分成五组。第一组是静态基线：固定 batch size、FIFO 请求顺序、不读取 HTTP 特殊时延要求，记录不同并发水平下的 rps、平均 E2E latency、P90/P99 TTFT、TPOT、SLA 达标率和高优先级请求违约率。第二组是外层 admission 与请求重排：并发数在给定测试范围内随机变化，设置高优先级、普通和低优先级请求比例，比较 FIFO、固定等待窗口、自适应等待窗口、SLO-aware 排序和 aging 策略。第三组是 runtime batching 能力叠加：在相同请求流下打开或调整 dynamic batch / continuous batching 相关参数，观察 active batch occupancy、KV 水位、ITL 和平均 E2E。第四组是 chunked prefill 或长请求隔离实验：构造短 prompt 与长 prompt 混合 workload，比较不同 chunk size、prefill/decode ratio、长请求单独队列对 TTFT、ITL 和整体 SLA 的影响。第五组是可选 PD 分离实验：仅在部署环境具备多个 prefill/decode 实例池和可观测 KV handoff 的条件下执行，重点测 KV transfer、阶段队列等待和跨池调度收益。
