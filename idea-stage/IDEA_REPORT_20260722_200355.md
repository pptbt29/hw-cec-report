...
| 算法难度 | 当前不足 | 单 session、未来 turn/长度已知、负载和网络 trace 已知，使 DP/MPC 接近小规模枚举 |
...
### 3.1 单 session 移除了 KV Manager 最困难的决策

单 session 下，如果后台带宽和显存足够，“把最可能目的地尽量复制完整”往往已经接近答案。真正非平凡的问题来自多个 session、多个候选节点和前台流量共同竞争有限 HBM、NIC 和后台窗口。

**多 session 共享资源不是后续扩展，而应是主问题的一部分。** 否则 Manager 缺少必须作取舍的稀缺资源，算法复杂度主要来自记账，而非决策。

### 3.2 未来信息过强，长期 Router 接近 oracle

当前实现读取真实剩余请求、入口、输入和输出长度做有限时域估值。它适合作为 oracle baseline，不适合作为可部署算法的核心证据。更重要的是，现有结果中长期 Router 尚未稳定优于单请求穷举，说明“长期价值”仍是待证假设，而不是已经成立的贡献。
...
2. **可部署的不确定信息模型**：未知 continuation、返回时间、目的节点和 KV 增长；真实未来只能用于 oracle。
...
- 在线策略不读取真实剩余 turn、真实未来入口或真实输出长度；
